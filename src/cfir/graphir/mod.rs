...
use self::instruction::{BindOperator, Branch, Conds, Instruction, Operator, Switch, Terminator};
use super::{
  base::Env,
  handles::{ConstantValue, DefineSymbol, GlobalSymbol, LabelSymbol, SymbolRef},
  types::{FirstClassType, FunctionAttr, FunctionType, GetType, PointerType, SimpleType, Type},
};
...
impl FunctionDef {
  pub fn make_control_flow_graph(&self) -> Vec<(LabelSymbol, LabelSymbol)> {
    ...
    let mapping = bbs
      .iter()
      .enumerate()
      .map(|(offset, x)| (x.label.clone(), offset))
      .collect::<HashMap<_, _>>();
  ...
  pub fn get_call_targets(&self) -> HashSet<SymbolRef> {
    ...
    bbs.iter().flat_map(|x| x.get_call_targets()).collect()
  }
}
...
impl BasicBlockDef {
  ...
  pub fn get_next(&self) -> Option<Vec<LabelSymbol>> {
    let r: Option<Vec<LabelSymbol>> = self.terminator.as_ref().map_or(Some(vec![]), |x| match x {
      Terminator::Branch(Branch(_, _, t, e)) => Some(vec![t.clone(), e.clone()]),
      ...
      Terminator::Switch(Switch(_, bs)) => Some(bs.iter().map(|x| x.1.clone()).collect()),
      Terminator::Unrechable | Terminator::Ret(_) => None,
    });
    debug!("get_next: {:?}", r);
    r
  }
