...
use crate::cfir::graphir::instruction::{
  BindOperator, Branch, Conds, Instruction, Operator, Ret, Store, Switch, Terminator,
};
use crate::cfir::{
  graphir::{BasicBlockDef, FunctionDef},
  handles::{LabelSymbol, LocalSymbol, SymbolRef},
};
...
impl RootLiveAnalysis for FunctionDef {
  ...
  fn live_analysis(&self) -> FunLiveVar {
    ...
    let vars: BBSLiveVar = get_all_veriable(self)
      .into_iter()
      .map(|x| (x, false))
      .collect();
    let mut old: FunLiveVar = self
      .bbs
      .iter()
...
pub fn once_pass(fun_def: &FunctionDef, mut inp: FunLiveVar) -> FunLiveVar {
  ...
  while !next_set.is_empty() {
    ...
    if !used_bb.contains(&task) {
      ...
      inp.insert(
        task.clone(),
        bb.live_analysis(inp.get(&task).unwrap().clone()),
      );
...
pub fn use_variable_for_terminator(ter: &Terminator, record: &mut BBSLiveVar) {
  ...
  match ter {
    Terminator::Branch(Branch(_, v, _, _)) | Terminator::Switch(Switch(v, _)) => {
