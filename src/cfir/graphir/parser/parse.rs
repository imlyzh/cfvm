...
impl ParseFrom<Rule> for AllocaType {
  fn parse_from(pair: Pair<Rule>) -> Self {
    ...
    let pair = pair.into_inner().next().unwrap();
    match pair.as_rule() {
      Rule::alloca_type_stack => {
        AllocaType::Stack(store_type_opt_parse_from(pair.into_inner().next().unwrap()))
      },
...
fn insts_parse_from(pair: Pair<Rule>) -> Vec<Instruction> {
  ...
  pair.into_inner().map(Instruction::parse_from).collect()
}
...
pub fn gen_label() -> LabelSymbol {
  LabelSymbol(Symbol::new(format!(
    "GEN_{}",
    LABEL_NUMBER.fetch_add(1, Ordering::SeqCst)
  )))
}
...
impl ParseFrom<Rule> for Branch {
  fn parse_from(pair: Pair<Rule>) -> Self {
    ...
    Branch(branch_op, cond, true_block, false_block)
  }
}
...
// module pars
fn env_parse_from(pair: Pair<Rule>, record: &mut Env<FunctionDef>) {
...
pub fn file_parse(input: &str, record: &mut Env<FunctionDef>) -> Result<(), Error<Rule>> {
